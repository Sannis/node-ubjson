...
strtok.DOUBLE_BE = {
    ...
    put : function(b, o, v, flush) {
      ...
      assert.equal(typeof v, 'number');
      var absV = Math.abs(v)
      assert.ok(2.23e-308 < absV && absV < 1.80e308);
      assert.ok(o >= 0);
      assert.ok(this.len <= b.length);
...
util.inherits(SourceStream, EventEmitter);
    ...
    case 'number':
      if (~~v == v) {
        if (-128 <= v && v <= 127) {
          ...
          throw new Error('Huge numbers are not supported yet');
        }
      } else {
        var absV = Math.abs(v);
        ...
        } else if (2.23e-308 < absV && absV < 1.80e308) {
    ...
    case 'object':
      if (v === null) {
        ...
        vk.forEach(function(k) {
          len += generalPack(s, b, bo + len, k, flush);
          len += generalPack(s, b, bo + len, v[k], flush);
        });
      }
